...
namespace ep24.web.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        {
            return orderRepo.List(it => it.PaidDate != null);
        }

        [HttpPost]
        public OrderProductResponse OrderProduct([FromBody]OrderProductRequest request)
        {
            if(request == null || request.OrderedProducts == null || request.OrderedProducts.Count() == 0) return new OrderProductResponse { Message = "ไม่พบเมนูที่จะสั่ง", };
            ...
            return new OrderProductResponse { Message = "สั่งเมนูสำเร็จ กรุณาชำระเงินที่เค้าเตอร์", ReferenceCode = order.Id.Substring(0,5)};
        }
    }
}
